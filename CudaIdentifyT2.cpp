...
class CudaCallExpressionCallback : public MatchFinder::MatchCallback {
  ...
  virtual void run(const MatchFinder::MatchResult &Result) {
    ...
    Rewrite.InsertText(startOfFile, "#include \"freeLaunch_T2.h\"\n",true,false);
    ...
    */
    ...
    string recordKernelInfo = FL_lc + FL_childBlockSize + FL_childKernelArgSz + _tmp_p + _tmp_childGridSize +_tmp_pmcpy + FL_check;
    ...
    Rewrite.InsertText(parentFuncDecl->getBody()->getLocEnd(), "FL_T2_Postloop;\n", true,false);
    ...
    */
    ...
    replace(childBodyStr,"threadIdx.x + blockIdx.x*blockDim.x;","k * FL_childBlockSize +threadIdx.x%FL_childBlockSize;");
    replace(childBodyStr,"return;","continue;");

    //Add the contents of the child kernel function definition to where it is called
    string loopTohandleChildTask = "\nfor(int k=0; k< kernelSz; k++)"+childBodyStr.substr(0, childBodyStr.length() - 1)+"\n";
...
public:
  ...
  void EndSourceFileAction() override {
    ...
    llvm::raw_fd_ostream outFile("/home/ubuntu/Project2_freeLaunch/Examples/mst_dp_modular_1/mainT2.cu", error_code, llvm::sys::fs::F_None);
